...
function getEquipmentStatBonus(itemAndPassives, stat, doCap = true, tdwCap = 1) {
    if ((constants.baseStats.includes(stat) || stat == "accuracy") && itemAndPassives[0] && constants.weaponList.includes(itemAndPassives[0].type)) {
        let normalStack = 0;
...
function calculateStatValue(itemAndPassives, stat, unitBuild) {
    ...
    var baseValue = 0;
    var buffValue = 0
    if (constants.baseStats.includes(stat)) {
...
function isEquipedConditionOK(equiped, condition) {
    if (Array.isArray(condition)) {
        ...
        if (constants.elementList.includes(condition)) {
        ...
        } else if (constants.typeList.includes(condition)) {
...
var dump = fs.readFileSync("./builder/GL/unitsWithSkill.json");
var unitsWithSkills = JSON.parse(dump);

dump = fs.readFileSync("./builder/GL/unitsWithPassives.json");
var units = JSON.parse(dump);

dump = fs.readFileSync("./builder/GL/defaultBuilderEspers.json");
var espers = JSON.parse(dump);

var builds = [];
var currentUnitIndex = 0;
function addNewUnit() {
    builds[builds.length] = new UnitBuild(null, [null, null, null, null, null, null, null, null,null,null,null], {});
...
for (var index = espers.length; index--;) {
    espersByName[espers[index].id] = espers[index];    
}
function fixItem(key, slotParam = -1, enhancements, pinItem = true) {
    ...
    if (typeof key === 'object') {
        ...
        if (constants.typeList.includes(key)) {
            ...
            for (var index = 0; index < 10; index++) {
                if (index != slot) {
                    ...
                    if (itemTmp  && !itemTmp.placeHolder && index != slot) {
                        var bestItemVersion = findBestItemVersion(builds[currentUnitIndex].build, 
                            itemTmp, dataStorage.itemWithVariation, builds[currentUnitIndex].unit);
...
module.exports = {
    loadStateHashAndBuild(data, importMode = false) {
        ...
        for (var i = 0; i < data.units.length; i++) {
            ...
            onUnitChange(data.units[i].id);
            
            if (unit.enhancementLevels) {
                builds[i].unit.enhancementLevels = unit.enhancementLevels;
                //displayUnitEnhancements();
                onUnitChange(data.units[i].id);
            }
            
            builds[i].goal = "custom";
            builds[i].formula = formula.parseFormula(unit.goal, unitsWithSkills[unit.id]);
            if (!formula.isSimpleFormula(builds[i].formula)) {
    ...
    getBuildStatsAsText(build) {
        var buildIndex = 0;
        var resultText = "Total: ";
        var first = true;
        for (var statIndex = 0, len = constants.baseStats.length; statIndex < len; statIndex++) {
            var result = calculateStatValue(build.build, constants.baseStats[statIndex], builds[buildIndex]).total;
            if (first) {
                first = false;
            } else {
                resultText += ", ";
