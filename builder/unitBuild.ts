    ...
    prepareEquipable(ignoreSlot = -1) {
        ...
        if (this.unit) {
            ...
            for (var equipIndex = 0, len = equip.length; equipIndex < len; equipIndex++) {
                if (constants.weaponList.includes(equip[equipIndex])) {
                    this.equipable[0].push(equip[equipIndex]);
                } else if (constants.shieldList.includes(equip[equipIndex])) {
                    this.equipable[1].push(equip[equipIndex]);
                } else if (constants.headList.includes(equip[equipIndex])) {
                    this.equipable[2].push(equip[equipIndex]);
                } else if (constants.bodyList.includes(equip[equipIndex])) {
            ...
            for (var i = this.unit.skills.length; i--;) {
                ...
                if (skill.equipedConditions) {
                    for (var j = skill.equipedConditions.length; j--;) {
                        if (!constants.typeList.includes(skill.equipedConditions[j]) && !constants.elementList.includes(skill.equipedConditions[j]) && !this.desirableItemIds.includes(skill.equipedConditions[j])) {
